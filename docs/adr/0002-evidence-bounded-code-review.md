...
Reviewer 返回最小 YAML：axis、范围指纹、`complete | stale | incomplete` 状态、reviewed entries、findings、limitations 与 checks。无效结果只携 validation errors 纠正一次，仍无效则该轴为 `incomplete`。最终结论只有“存在阻塞问题”“证据不足”“未发现阻塞问题”；Minor 不改变结论，任何结论都不代表满足仓库 merge policy。
